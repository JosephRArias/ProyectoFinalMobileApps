...
namespace ProyectoFinalMobile.Services
{
	public class GoogleMapsApiService : IGoogleMapsApiService
	{
		...
		public async Task<GooglePlaceAutoCompleteResult> GetPlaces(string text)
		{
			...
			using (var httpClient = CreateClient())
			{
				var response = await httpClient.GetAsync($"api/place/autocomplete/json?input={Uri.EscapeUriString(text)}&key={Config.ApiKey}").ConfigureAwait(false);
				if (response.IsSuccessStatusCode)
				{
					var json = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
		...
		public async Task<GooglePlace> GetPlaceDetails(string placeId)
		{
			...
			using (var httpClient = CreateClient())
			{
				var response = await httpClient.GetAsync($"api/place/details/json?placeid={Uri.EscapeUriString(placeId)}&key={Config.ApiKey}").ConfigureAwait(false);
				if (response.IsSuccessStatusCode)
				{
					var json = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
					if (!string.IsNullOrWhiteSpace(json) && json != "ERROR")
